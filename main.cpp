...
public:
    // ===== Utility functions to access stations safely =====
    ...
    void removeEdge(const string& station1, const string& station2) {
        if (hasStation(station1) && hasStation(station2)) {
            stations[station1].neighbors.erase(station2);
            stations[station2].neighbors.erase(station1);
        }
    }
    ...
    string Get_Minimum_Distance(const string& src, const string& dst) const {
        if (!hasStation(src) || !hasStation(dst)) {
        ...
        while (!pq.empty()) {
            ...
            for (auto& neighbor : stations.at(currentStation).neighbors) {
                int lineChangePenalty = 0;
                if (stations.at(neighbor.first).line != stations.at(currentStation).line)
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: {
                ...
                cin >> dist;
                cin.ignore();
                if (!metroMap.hasStation(from) || !metroMap.hasStation(to)) {
            ...
            case 7: {
                ...
                getline(cin, to);
                if (!metroMap.hasStation(from) || !metroMap.hasStation(to)) {
                    cout << "❌ Error: One or both stations do not exist.\n";
                } else {
                    metroMap.removeEdge(from, to);
